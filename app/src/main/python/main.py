...
INTRO_S = R"""
Kalutines: A tool to Complete homework in Onion School.
Copyright (C) 2023 Imken Luo

Licensed under GNU General Public License version 3 or later.
Homepage: https://github.com/immccn123/Kalutines
"""
...
def complete_topic(homework_id, topic):
    ...
    if topic["videoState"] == "unfinished":
        print("\t\t正在完成视频...", end="")
        submit_video(topic_id, homework_id, topic_detail["video"]["id"], int(topic_detail["video"]["duration"]), )
        print(" 完成")
    ...
    for pc in topic_detail["practices"]:
        ...
        if problem_type in ["multi_choice", "single_choice"]:
            ...
            bug_report("未知问题类型 10001", break_point="complete_topic", problem=problem, topic_name=topic["name"],
                ptype=problem_type, )
        done_count += 1
...
def complete_practice(homework_id, problem_list):
    for index, problem in enumerate(problem_list):
        ...
        if problem_type in ["single_choice", "multi_choice"]:
            ...
            bug_report("未知问题类型 10001", break_point="complete_practice", problem=problem, ptype=problem_type, )
        submit_practice_problem(homework_id,
            [{"problemId": problem["id"], "answer": ans, "duration": randint(1, 12), }],
...
def complete_exam(group_id, problem_list):
    ...
    for index, problem in enumerate(problem_list):
        ...
        if problem_type in ["single_choice", "multi_choice"]:
            ...
            bug_report("未知问题类型 10001", break_point="complete_exam", problem=problem, ptype=problem_type, )
        commit_problem_progress(group_id, index == len(problem_list) - 1,
            [{"problemId": problem["id"], "answer": ans, "type": problem_type, "duration": randint(1, 120), }], )
...
def complete_vacation_task_video(t, tl):
    ...
    if t["videoState"] == 0:
        print("\t正在完成视频...", end="")
        submit_vacation_video(topic["id"], topic["video"]["id"], t["id"], int(topic["video"]["duration"]) + 1, )
        print(" 完成")
    if t["problemState"] != 0:
        return
    ans = []
    ...
    for index, problem in enumerate(topic["practices"]):
        ...
        if problem["type"] in ["single_choice", "multi_choice"]:
            ...
            bug_report("未知问题类型 10001", timeline_name=tl["name"], break_point="complete_vacation", problem=problem,
                ptype=problem["type"], )
        submit_vacation_practice(problem["problemId"], topic["id"], t["id"], problem["pool"],
            index == len(topic["practices"]) - 1, ans, )
        print(f"\t完成练习 ({index + 1}/{practices_count})")


def complete_vacation_task_stage(t, vc, subject_id, stage_id):
    problems = get_vacation_stage_problem(t["id"], stage_id, subject_id, vc["id"], )
    problem_details = []
    ...
    for index, problem in enumerate(problems):
        ...
        if problem_type in ["single_choice", "multi_choice"]:
            ...
            bug_report("未知问题类型 10001", break_point="vacation", problem=problem, ptype=problem_type, )
        problem_details.append(submit_vacation_stage_problem(task_id=t["id"], stage_id=stage_id, subject_id=subject_id,
            homework_id=vc["id"], problem_id=problem["id"], problem_type=problem_type, answer=ans,
    ...
    for tl in timelines:
        ...
        for t in tasks:
            if t["type"] == 1:
                ...
                complete_vacation_task_stage(t, vc, subject_id=subject_id, stage_id=stage_id)


def run():
    unfinished_homework = get_unfinished_homework()
    expired_homework = get_expired_homework()
    homework: list = (unfinished_homework["homeworkList"] + expired_homework["homeworkList"])
